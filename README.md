...
- [Fashion Street AI — 长三角街巷时尚度评价平台](#fashion-street-ai--长三角街巷时尚度评价平台)
  - [目录](#目录)
  - [技术栈](#技术栈)
    - [前端](#前端)
    - [后端](#后端)
  ...
  - [前端页面与组件](#前端页面与组件)
    ...
    - [组件清单](#组件清单)
  - [API 接口一览](#api-接口一览)
    - [评价分析 (`/api/v1/analyze`)](#评价分析-apiv1analyze)
    - [任务进度 (`/api/v1/task`)](#任务进度-apiv1task)
    - [Prompt 模板 (`/api/v1/prompts`)](#prompt-模板-apiv1prompts)
    - [指标体系模板 (`/api/v1/metric-templates`)](#指标体系模板-apiv1metric-templates)
    - [分析中心显示配置 (`/api/v1/analytics-config`)](#分析中心显示配置-apiv1analytics-config)
    - [其他](#其他)
  - [环境变量与启动](#环境变量与启动)
    - [后端](#后端-1)
    - [前端](#前端-1)
...
# 初始化数据库 用法（在 backend 目录下，已配置好 .env）：
